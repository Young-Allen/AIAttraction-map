<template>
  <view class="mov-area-all" :style="{height: height}" @touchstart="onTouchstart" @touchmove="onTouchmove"
    @touchend="onTouchend">
    <view v-for="(item,index) in movList" class="mov-view-list" :class="item.indexClass" :style="{left: item.x + 'rpx'}"
      :key="index">
      <view class="mov-area-con" :class="item.conClass" @click="moveTo()">
...
<script>
  ...
  export default {
    name: 'MovableSwiper',
    computed: {
      ...mapState('m_roadMsg', ['activePoint']),
    },
    props: {
      /**
       * 传入的数据数组
    ...
    watch: {
      activePoint(newIndex, oldIndex) {
        ...
        let countPoint = newIndex - oldIndex
        if (countPoint > 0) {
          while (countPoint--) {
      ...
      dataIndex(newDataIndex, oldDataIndex) {
    ...
    methods: {
      ...mapMutations('m_roadMsg', ['saveActivePointToStorage', 'updateActivePoint']),

      moveTo(clientX, moveX) {
      ...
      onTouchmove(e) {
        ...
        if (this.movType == 0) {
          ...
          this.movList[this.touchIndex].x = (this.moveX - this.clientX - 750) <= 0 ? (this.moveX - this.clientX - 750) :
            0;
