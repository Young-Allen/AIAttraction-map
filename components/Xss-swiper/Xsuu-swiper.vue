  ...
  <view class="content-top">
    <swiper @change="swiperChange" :style="{height:height+'rpx'}" circular="true" autoplay="true" interval="8000"
      ...
      <swiper-item v-for="(item,index) in swiperItems" :key="index">
        <view class="content"
          :style="{height:height-10+'rpx',marginLeft:margin+'rpx',marginRight:margin+'rpx',borderRadius:borderRadius+'rpx'}"
          style="border: 5rpx solid white;">
          <image :src="item.img" mode="aspectFill" class="swiper-img"></image>
          <view class="mask"></view>
          <view class="written">
            <text v-if="item.title" class="Text-initialization overflow"
              :style="{fontSize: titleFontSize+'rpx',color:titleColor}" :class="[index === enjoySiwper ? 'title' : '']">
              {{item.title}}
            </text>
