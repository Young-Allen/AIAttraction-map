  ...
  <view class="scroll-wrap" :style="getScrollWrapStyle">
    ...
    <scroll-view v-if="!isShowList" class="scroll-view_hold" scroll-x="true" scroll-with-animation="true"
      ...
      <view v-for="(item, index) in list" :key="index" class="scroll-view-item"
        :style="choseInd === index ? getActiveStyle : getDefaultStyle" @click="onClickScrollItem(index, item)">
        {{ item }}
      </view>
      <view v-if="isUseOpenList" style="width: 62rpx; display: inline-block;">
      </view>
    </scroll-view>
    <!-- #endif -->
    <!-- #ifdef H5 -->
    <scroll-view v-show="!isShowList" class="scroll-view_hold" scroll-x="true" scroll-with-animation="true"
      :scroll-left="position">
      <view v-for="(item, index) in list" :key="index" class="scroll-view-item"
        :style="choseInd === index ? getActiveStyle : getDefaultStyle" @click="onClickScrollItem(index, item)">
        {{ item }}
    ...
    <!-- #endif -->
    <view v-if="isUseOpenList && !isShowList" class="open-list" @click="handleShowList">
    ...
    <view v-show="isShowList" class="list-wrap bg-fff">
      ...
      <view class="open-list-content flex flex-wrap padding-lr padding-bottom-sm">
        <view v-for="(item, index) in list" :key="index" :style="choseInd === index ? getActiveStyle : getDefaultStyle"
          class="scroll-view-item_wrap" @click="onClickListItem(index, item)">
          {{ item }}
        </view>
      </view>
    </view>
