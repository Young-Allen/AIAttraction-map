...
function sustained(that) {
  ...
  setInterval(function() {
    ...
    for (let number = 0; number < data.length; number++) {
      ...
      data[number].floating[0] < 15 ? data[number].floating[0] = data[number].floating[0] + 1 : data[number]
        .floating[0] = 1;
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (i != number) {
          ...
          if (index_1 < index_2) {
            ...
            (data[i].x + data[i].radius) > value.boundary[0] ? data[i].x = (value.boundary[0] - data[i].radius) :
              data[i].x;
            (data[i].y + data[i].radius) > value.boundary[1] ? data[i].y = (value.boundary[1] - data[i].radius) :
              data[i].y;
...
function mapping(_this) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (data[i].labelName.length < 6) {
      context.fillText(data[i].labelName, (data[i].x - (data[i].size * (0.5 * data[i].labelName.length))), (data[i].y +
        data[i].size * (0.5)))
    } else {
      context.fillText(data[i].labelName.substring(0, 3), (data[i].x - (data[i].size * (0.5 * (data[i].labelName
          .substring(0, 3)).length))),
        ((data[i].y + data[i].size * (0.5)) - data[i].size * (0.8)))
      context.fillText(data[i].labelName.substring(3, 6), (data[i].x - (data[i].size * (0.5 * (data[i].labelName
          .substring(3, 6)).length))),
        ((data[i].y + data[i].size * (0.5)) + data[i].size * (0.8)))
    }
  }
  context.draw();
